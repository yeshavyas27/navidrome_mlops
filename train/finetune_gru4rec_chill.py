    ...
    with mlflow.start_run(run_name=run_name) as run:
        ...
        for epoch in range(1, ft_cfg["epochs"] + 1):
            ...
            if epoch % eval_every == 0 or is_last:
                ...
                # ---------------------------------------------------------- #
                # Push every evaluated checkpoint to MinIO unconditionally.   #
                # The artifact key encodes the epoch number so each push is a #
                # distinct object and no previous checkpoint is overwritten.  #
                # ---------------------------------------------------------- #
                _is_best = results["session_HR"] > best_session_hr
                if MINIO_AVAILABLE:
                    _current_state = {k: v.cpu().clone() for k, v in model.state_dict().items()}
                    _meta = {
                        "run_type":              "finetune",
                        "mlflow_run_id":         run.info.run_id,
                        "timestamp":             datetime.now(timezone.utc).isoformat(),
                        "session_HR":            round(results["session_HR"], 6),
                    ...
                    try:
                        ...
                        # epoch is forwarded so push_run_artifacts can embed it
                        # in the MinIO key, e.g.:
                        #   finetune/{date}/{run_id}/epoch_007/model.pt
                        _keys = push_run_artifacts(
                            state_dict=_current_state,
                            run_type="finetune",
                        ...
                        # Always update the "latest" tags so the MLflow run
                        # always points at the most recently uploaded checkpoint,
                        # and also record a per-epoch tag for full traceability.
                        mlflow.set_tags({
                            "minio_model_key":              _keys["model_key"],
                            "minio_metadata_key":           _keys["metadata_key"],
                            f"minio_epoch_{epoch:03d}_key": _keys["model_key"],
                        })
                        log.info(
                            f"  [minio] epoch {epoch:03d} "
                            f"{'(best) ' if _is_best else ''}"
                            f"model    → {_keys['model_key']}"
                        )
                        log.info(
                            f"  [minio] epoch {epoch:03d} "
                            f"metadata → {_keys['metadata_key']}"
                        )
                    except Exception as _e:
                        log.warning(f"  [minio] Upload skipped (epoch {epoch}) — {_e}")
